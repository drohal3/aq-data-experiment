...
PATH_A = "local/recent_30m_all_attr"

data_dynamo_a = []
with open(f'{PATH_A}/DynamoDBRetriever_30m.txt', 'r') as file:
...
data_kinesis_a = []
with open(f'{PATH_A}/KinesisRetriever_30m.txt', 'r') as file:
    # Read each line in the file
    for line in file:
        # Strip newline characters and print each line
        json_line = json.loads(line)
        data_kinesis_a.append(json_line["response_stats"]["elapsed"])

data_timestream_a = []
with open(f'{PATH_A}/TimestreamDBRetriever_30m.txt', 'r') as file:
    # Read each line in the file
    for line in file:
        # Strip newline characters and print each line
        json_line = json.loads(line)
...
data_a = [data_dynamo_a, data_kinesis_a, data_timestream_a]
